...
class EvaluationsDownloadController extends Controller{
    use CommonTrait;
     ...
     */
    public function downloadReport($instance_id): RedirectResponse | BinaryFileResponse{
        try{
            $instance = Instance::where('id', '=', $instance_id)->first();
            /** Get evaluation */
            $evaluation = Evaluation::where('type', '=','__training')->where('model_id', '=', $instance->id)->first();
            ...
            /** Set name and date */
            $sheet->setCellValue('A4', $instance->trainingRel->title);
            $sheet->setCellValue('I4', $this->onlyDate($instance->startDate()));

            $secondSheet->setCellValue('A4', $instance->trainingRel->title);
            $secondSheet->setCellValue('I4', $this->onlyDate($instance->startDate()));
            ...
            foreach($options as $option){
                ...
                $row += 2;
                $answers = EvaluationAnswer::whereHas('statusRel', function($query){
                ...
                foreach ($answers as $answer){
                    if($answer->answer != 0){
                        ...
                        $sheet->setCellValue('B' . $row, $answer->userRel->name ?? 'Anonimno');
            ...
            foreach($otherOptions as $option){
                ...
                foreach ($answers as $answer){
                    if($answer->answer != ''){
                        ...
                        $secondSheet->setCellValue('B' . $row, $answer->userRel->name ?? 'Anonimno');
            ...
            $fileName = ($this->generateSlug($instance->trainingRel->title . '_' . date('d-m-y'))) . '.xlsx';
