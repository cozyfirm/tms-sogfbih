...
@endsection
@section('c-buttons')
    <a href="{{ route('system.admin.other.analysis.submodules.evaluations.preview', ['analysis_id' => $analysis->id ]) }}" title="{{ __('Nazad na pregled pitanja') }}">
    ...
    @isset($preview)
        @if(!$evaluation->locked)
            <a href="{{ route('system.admin.other.analysis.submodules.evaluations.edit-option', ['id' => $option->id ]) }}">
                <button class="pm-btn pm-btn-white btn pm-btn-edit">
                    <i class="fas fa-edit"></i>
                </button>
            </a>
            <a href="{{ route('system.admin.other.analysis.submodules.evaluations.delete-option', ['id' => $option->id ]) }}">
...
@section('content')
    <div class="content-wrapper content-wrapper-p-15">
        <div class="row">
            <div class="col-md-12">
                <form action="@if(isset($edit)) {{ route('system.admin.other.analysis.submodules.evaluations.update-option') }} @else {{ route('system.admin.other.analysis.submodules.evaluations.save-option') }} @endif" method="POST" id="js-form">
                    @if(isset($edit))
                        {{ html()->hidden('id')->class('form-control')->value($option->id) }}
                    @endif

                    {{ html()->hidden('analysis_id')->class('form-control')->value($analysis->id) }}
